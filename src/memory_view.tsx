...
function MemoryLine({ memory, index, length }: {memory: number[], index: number, length: number}): ReactElement {
    ...
    for(let i = 0; i < length; i++) {
        const cell_index = index + i;
        let class_name = "";
        const access_type = last_accesses.get(cell_index);
        if(access_type !== undefined) {
            class_name = "last-" + access_type;
        }
        const cell = <td key={i} className={class_name}>{memory[cell_index].toString(16).padStart(2, "0")}</td>;
        ret.push(cell);
    }
